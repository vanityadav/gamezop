...
export default function GameCard({ game, language = "en" }: Props) {
  ...
  return (
    <div className="grid  text-sm gap-3 text-heading">
...
const Rating = ({ rating }: { rating: number }) => {
  ...
  <div className="flex items-center max-w-max">
    {rating}
    <Star strokeWidth={0} className="h-4 w-4 fill-heading" />
  </div>;
};
  ...
  <div className="text-balance text-xs">
    {categories.map((category, index) => (
      <span
        ...
        className={
          index >= 1
            ? "ml-5 relative before:absolute before:-left-3 before:top-1/2  before:h-1 before:w-1 before:rounded-full before:bg-heading"
            : ""
        }
      >
        {category}
